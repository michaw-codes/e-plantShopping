...
function ProductList({ onHomeClick }) {
    ...
    const plantsInCartAmount = Object.keys(addedToCart).length;
    const dispatch = useDispatch();
    const cartItems = useSelector((state) => state.cart.items);

    useEffect(() => {
        setAddedToCart(cartItems.reduce((addedItems, item) => {
                    ...
                    <div>
                        <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
                            <h3 className='cart'>
                                ...
                                <span className="cart_quantity_count">{plantsInCartAmount}</span>
                            </h3>
                        </a>
                    </div>
                </div>
            ...
            {!showCart ? (
                ...
                <div className="product-grid">
                    {plantsArray.map((plantGroup, groupIndex) => (
                        <div key={groupIndex}>
                            <h2>{plantGroup.category}</h2>
                            <div className="product-list">
                                {plantGroup.plants.map((plant, index) => {
                                    const plantAddedToCart = Boolean(addedToCart[plant.name]);
                                    return (
                                        <div className="product-card" key={index}>
                                            ...
                                            <button
                                                className={plantAddedToCart ? "product-button added-to-cart" : "product-button"}
                                                onClick={() => handleAddToCart(plant)}
                                            >
                                                {plantAddedToCart ? "Added to Cart" : "Add to Cart"}
                                            </button>
                                        </div>
                                    )
                                })}
